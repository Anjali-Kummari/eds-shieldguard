...
async function buildNav(block) {
  ...
  brand.innerHTML = `
    <svg
      ...
      style="color:var(--color-brand-primary);width:2rem;height:2rem;"
    >
  ...
  const allLinks = [...navDoc.querySelectorAll('a')];

  // Add navigation links
  allLinks.forEach((a, index) => {
    // Skip first link (logo)
    // Skip last link (CTA)
    if (index === 0 || index === allLinks.length - 1) return;
    ...
    if (
      ...
      new URL(a.href, window.location.origin).pathname
    ) {
  ...
  cta.href = lastLink.href;
  cta.className = 'nav-cta';
  cta.textContent = lastLink.textContent.trim();
  ...
  allLinks.forEach((a, index) => {
    ...
    // Last item becomes CTA
    if (index === allLinks.length - 1) {
      link.className = 'nav-mobile-cta';
    }
