...
function buildStep1(selected) {
  return `
    <div class="quote-step active" id="quote-step-1">
      <label class="form-label" style="margin-bottom:var(--space-4)">
    ...
    },
    {
      id: 'business',
      label: 'Business',
      icon: '<rect x="2" y="7" width="20" height="14" rx="2"/><path d="M16 7V5a2 2 0 00-2-2h-4a2 2 0 00-2 2v2"/><line x1="12" y1="12" x2="12" y2="17"/><line x1="9" y1="14.5" x2="15" y2="14.5"/>',
    },
    {
      id: 'other',
      label: 'Other',
      icon: '<path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/>',
          ...
          <button
            class="quote-type-btn${selected === t.id ? ' selected' : ''}"
            data-type="${t.id}"
          >
            <svg
  ...
  return `
    <div class="quote-step active" id="quote-step-2">
      <div class="quote-field">
        ...
        <div>
          <label class="form-label" for="quote-value">
  ...
  return `
    <div class="quote-step active" id="quote-step-3">
      <div class="quote-field">
        ...
        >
          By clicking "Get Estimate" you agree to our terms.
          We'll use your data to calculate the best available rate.
        </p>
      </div>
    </div>`;
}
    ...
    <div class="quote-step active" id="quote-step-4">
      <div class="quote-result">
        ...
        <div class="quote-premium-box">
          ...
          <div class="quote-premium-amount">
            ${data.estimatedMonthlyPremium || '$89–$124'}
          </div>
        </div>
        ...
        ${data.advice
    ? `<div class="quote-advice">"${data.advice}"</div>`
    : ''}
...
function mockRecommendation(formData) {
  ...
  return {
    ...
    advice: `Based on your ${formData.type} in ${
      formData.zip || 'your area'
    }, this coverage level offers an excellent balance of protection and value.`,
  };
}
...
export default function decorate(block) {
      ...
      <div class="quote-body">
        <div class="quote-header">
          ...
          <div class="quote-step-label">
            <span id="quote-step-counter">
              Step ${step} of ${STEPS}
            </span>

            <div class="quote-progress">
              <div
                class="quote-progress-fill"
                id="quote-progress-fill"
        ...
        <div id="quote-step-container">
          ${buildStep1(formData.type)}
        </div>

        <div class="quote-nav">
          <button class="quote-back hidden" id="quote-back">
            Back
          </button>
  ...
  function renderStep() {
    ...
    if (step === 1) {
      html = buildStep1(formData.type);
    }

    if (step === 2) {
      html = buildStep2(formData.zip, formData.value);
    }

    if (step === 3) {
      html = buildStep3(formData.email);
    }

    if (step === 4) {
      html = buildStep4Loading();
    }
    ...
    if (step === 1) {
      container.querySelectorAll('.quote-type-btn').forEach((btn) => {
        btn.addEventListener('click', () => {
          ...
          container.querySelectorAll('.quote-type-btn').forEach((b) => {
  ...
  nextBtn.addEventListener('click', () => {
    if (step === 2) {
      formData.zip = block.querySelector('#quote-zip')?.value.trim()
        || formData.zip;

      formData.value = block.querySelector('#quote-value')?.value.trim()
        || formData.value;
    }

    if (step === 3) {
      formData.email = block.querySelector('#quote-email')?.value.trim()
        || formData.email;
    }
