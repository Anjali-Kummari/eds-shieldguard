  ...
  block.innerHTML = `
    <div class="resource-center-inner">
      <div class="resource-header">
        ...
        <p>Expert advice, insurance guides, and tips to help you protect what matters most.</p>
      </div>
...
export default function decorate(block) {
  ...
  if (rows.length >= 6) {
   ...
   for (let i = 0; i + 5 < rows.length; i += 6) { 
     parsed.push({ id: String(id++), category: rows[i]?.children[1]?.textContent.trim() || 'General', title: rows[i + 1]?.children[1]?.textContent.trim() || '', excerpt: rows[i + 2]?.children[1]?.textContent.trim() || '', content: rows[i + 3]?.children[1]?.innerHTML || '', date: rows[i + 4]?.children[1]?.textContent.trim() || '', readTime: rows[i + 5]?.children[1]?.textContent.trim() || '5 min read', image: rows[i + 5]?.children[1]?.querySelector('img')?.src || 'https://images.unsplash.com/photo-1560518883-ce09059eeffa?auto=format&fit=crop&q=80&w=800', 
                 }); 
   }
